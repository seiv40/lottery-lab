...
# -*- coding: utf-8 -*-
"""
Deep Sets v2 for Module 5: Deep Learning Model Zoo

This is a capacity test. DeepSets v1 is simple: linear layers + ReLU.
DeepSets v2 adds:
- Residual connections (helps gradients flow)
- Multi-head aggregation (sum + mean + max)
- LayerNorm (stabilizes training)
- Deeper phi network

The question: does extra capacity help?

For random lottery data, we expect NO IMPROVEMENT. If there's no signal to
learn, more parameters just give the model more ways to overfit noise.

If v2 significantly outperforms v1, that's actually suspicious - it would
suggest there ARE patterns that need the extra model capacity to capture.
...
CAND_PATTERNS = [
    ...
    r".*regular.*[1-5].*",  # regular_x variations
]


def detect_ball_columns(df: pd.DataFrame) -> List[str]:
    """
    Try to find 5 "ball-like" columns. If we cannot, fall back to the first
    5 numeric feature columns (excluding obvious metadata like dates, jackpot).

    Returns:
        List of 5 column names.
    """
    ...
    if len(matched) >= 5:
        ...
        print(f"[DeepSets v2] Detected ball columns by pattern: {selected}")
        return selected

    # heuristic: any numeric columns that look like counts / indices
    numeric_cols = []
    for c in cols:
    ...
    if len(numeric_cols) >= 5:
        ...
        print(f"[DeepSets v2] Fallback to first 5 numeric columns: {selected}")
        return selected

    raise ValueError(
        f"[DeepSets v2] Could not detect 5 ball columns; numeric candidates = {numeric_cols}"
    )
...
# DeepSets v2 model definition; remains unchanged

class ResidualBlock(nn.Module):
    """Simple residual MLP block with LayerNorm."""
...
class Phi(nn.Module):
    ...
    - Two residual blocks
    """
    def __init__(self, embed_dim: int = 64, hidden: int = 128, dropout: float = 0.1):
        super().__init__()
        self.fc_in = nn.Linear(1, hidden)
        self.ln_in = nn.LayerNorm(hidden)
        self.res1 = ResidualBlock(hidden, dropout=dropout)
        self.res2 = ResidualBlock(hidden, dropout=dropout)
        self.fc_out = nn.Linear(hidden, embed_dim)
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: [B, 5] -> embeddings: [B, 5, embed_dim]
        """
...
class DeepSetsV2(nn.Module):
    """
    Deep Sets v2 with heteroscedastic Gaussian head and multi-aggregation.
    """
    def __init__(self,
                 embed_dim: int = 64,
                 phi_hidden: int = 128,
        ...
        self.phi = Phi(embed_dim=embed_dim, hidden=phi_hidden, dropout=dropout)

        pooled_dim = embed_dim * 3  # sum, mean, max

        self.rho_fc1 = nn.Linear(pooled_dim, rho_hidden1)
    ...
    def forward(self, x_set: torch.Tensor):
        ...
        # multi-aggregation over the set dimension
        z_sum = z.sum(dim=1)     # [B, embed_dim]
        z_mean = z.mean(dim=1)   # [B, embed_dim]
        z_max, _ = z.max(dim=1)  # [B, embed_dim]

        z_cat = torch.cat([z_sum, z_mean, z_max], dim=-1)  # [B, 3*embed_dim]

        h = self.rho_fc1(z_cat)
        h = self.rho_ln1(h)
        h = F.relu(h)
        h = self.dropout(h)
...
def train_and_export(dm, args) -> Dict:
    ...
    phi_hidden = int(getattr(args, "ds_phi_hidden", 128))
    rho_hidden1 = int(getattr(args, "ds_rho_hidden1", 96))
    rho_hidden2 = int(getattr(args, "ds_rho_hidden2", 64))
...
# truncate the validation and test sets to match
    ...
    # 8. --- Standardize FEATURES (the ball columns) ---
    ...
    for ep in range(max_epochs):
        ...
        if getattr(args, "verbose", False) and ((ep + 1) % 50 == 0 or ep == 0):
            print(
                f"[DeepSets v2][{lottery}] epoch {ep+1}/{max_epochs} "
...
# re-align y_true just in case loader dropped last batch
        ...
        y_lo = y_pred - half_width
        y_hi = y_pred + half_width
    ...
    perf_path = out_dir / f"deepset_v2_perf{suffix}.json"
    ...
    pred_path = out_dir / f"deepset_v2_predictive{suffix}.json"
    pred_path.write_text(
        json.dumps(pred_payload, indent=2), encoding="utf-8"
    )

    ds_dir = MODELS_DIR / "deepsets_v2"
    ds_dir.mkdir(parents=True, exist_ok=True)
    weights_path = ds_dir / f"{lottery}_deepsets_v2.pt"
    torch.save(model.state_dict(), weights_path)

    summary_path = write_summary("deepsets_v2", lottery, metrics, args=args)
    
    print(f"[DeepSets v2] Training complete. Test RMSE: {rmse:.4f}, Coverage: {coverage:.4f}, Test Samples: {len(y_true)}")
...
# Main block; argument parsing remains unchanged

if __name__ == "__main__":
    import argparse
    from utils_data import LotteryFeatureDataModule

    parser = argparse.ArgumentParser(description="Train DeepSets v2 model.")
...
# model specific
    ...
    parser.add_argument("--ds_phi_hidden", type=int, default=128)
    parser.add_argument("--ds_rho_hidden1", type=int, default=96)
    parser.add_argument("--ds_rho_hidden2", type=int, default=64)
...
# add dummy args for compatibility
    ...
    print(f"[DeepSets v2] Model: deepsets_v2 | Lottery: {args.lottery}")
...
# we must add the file suffixes to the args for write_summary
    ...
    try:
        ...
        print(f"[DeepSets v2] Training complete. Performance report: {result.get('perf_path')}")
    except Exception as e:
        print(f"[DeepSets v2] Worker crashed:\n{e}")
        import traceback
        traceback.print_exc()

    print("[DeepSets v2] Done.")